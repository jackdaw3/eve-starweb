        ...
        <el-table :data="filterTableData" :table-layout="tableLayout" :highlight-current-row="highlight"
            ...
            <el-table-column type="expand">
                <template #default="props">
                    <el-table :data="props.row.Materials" style="width: 55%" :table-layout="tableLayout"
                        :cell-style="tableStyle" :span-method="objectSpanMethod" :row-class-name="handelMaterialRowDetail"
                        ...
                        <el-table-column prop="Quantity" :label="t('table.material.quantity')" :formatter="numberFormat"
                            min-width="10%" />
                        ...
                        <el-table-column min-width="20%">
                            <template #header>
                                <el-button size="small" @click="copyAllMaterials(props.row.Materials)" plain>{{
            ...
            <el-table-column :width="operationWidth" min-width="10%">
                <template #header align="right">
                    <el-input v-model="search" size="default" :placeholder="t('table.lookUp')" :prefix-icon="Search" clearable/>
                </template>
                <template #default="scope">
...
const form = computed(() => store.state.loyalty.form)
const calculator = computed(() => store.state.loyalty.tableForm.calculator === "true");
const tableLayout = computed(() => store.state.loyalty.tableForm.layout);
const highlight = computed(() => store.state.loyalty.tableForm.highlight === "true");
const regionId = computed(() => store.state.loyalty.regionId)
const corporationId = computed(() => store.state.loyalty.corporationId)
...
onBeforeRouteLeave(() => {
    store.commit('loyalty/setCorporationId', 0);
});
...
function loadTableData() {
    loading.value = true
    axios.get(`${BACKEND_SERVER}lp/offer`, {
        params: {
        ...
        .then(response => {
            for (let i = 0; i < response.data.length; ++i) {
                var materials = response.data[i].Materials;
                if (materials == null) {
                    continue;
                }
                let count = 0;
...
function handelRowDetail({ row, rowIndex }: { row: Item; rowIndex: number }): string {
    let value = '';
    if (row.Materials === null || row.Materials.length === 0) {
...
function handelMaterialRowDetail({ row, rowIndex }: { row: Item; rowIndex: number }): string {
    let value = '';
    if (row.Error === true) {
        value += "warning-row";
    }
...
function openCalculator() {
    ...
    store.commit('loyalty/setCalculatorVisible', true);
    store.commit('loyalty/setSelectedItems', selectedItems);
}
