...
const calculatorVisible = computed({
    get: () => store.state.loyalty.calculatorVisible,
    set: (value) => store.commit('loyalty/setCalculatorVisible', value)
});
let selectedItems = computed(() => store.state.loyalty.selectedItems);
...
function processMaterials() {
    ...
    selectedItems.value.forEach((item: Item, index: number) => {
        ...
        const materialsWithResetLength = item.Materials.map(material => ({
            ...material,
            length: 0,
