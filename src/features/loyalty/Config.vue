...
onMounted(() => {
    ...
    if (storedForm) {
        ...
        store.commit('loyalty/setForm', { ...form });
    }
...
watch(corporationIds, (newCorporationIds) => {
    if (newCorporationIds) {
        store.commit('loyalty/setCorporationId', parseInt(newCorporationIds[1]));
    } else {
        store.commit('loyalty/setCorporationId', undefined);
    }
})
...
async function loadFactions() {
    try {
        const response = await axios.get(`${BACKEND_SERVER}lp/faction`, {
            params: {
                lang: locale.value,
            },
        });
        loadCorporations(response.data);
        processCorporationsData(response.data);
    } catch (error) {
        console.error(error);
    }
}
...
function closeDialog() {
    ...
    function formchange() {
        const oldForm = store.state.loyalty.form
    ...
    function tableFormChange() {
        const oldTableForm = store.state.loyalty.tableForm
